...
class MNewsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        query1 = '인텔'
        sd = (date.today() - timedelta(days=1)).strftime('%Y%m%d') # 검색을 시작할 날짜
        ed = (date.today() - timedelta(days=1)).strftime('%Y%m%d') # 검색을 종료할 날짜
#         sd = '20200301'
#         ed = '20200301'
        print(f'start day: {sd}')
...
#         pub_date = response.meta['date']
        pub_date = (date.today() - timedelta(days=1)).strftime('%Y-%m-%d')
#         pub_date = '2020.03.01.'
